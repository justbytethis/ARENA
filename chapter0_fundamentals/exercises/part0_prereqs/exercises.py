...
# %%
arr2 = einops.repeat(arr[0], "c h w -> c h (n w)", n=2)
display_array_as_img(arr2)
# %%
arr3 = einops.repeat(arr[0:2], "b c h w -> c (b h) (n w)", n=2)
display_array_as_img(arr3)
# %%
arr4 = einops.repeat(arr[0], "c h w -> c (h n) w", n=2)
...
# %%
arr8 = einops.reduce(arr6, "c (h2 2) (w2 2) -> c h2 w2", "max")
...
def rearrange_1() -> Tensor:
    ...
    """
    ts = t.arange(3,9)
    ts = einops.rearrange(ts, "(a b) -> a b", a=3)
    return ts
...
def rearrange_2() -> Tensor:
    ...
    """
    ts = t.arange(1,7)
    return einops.rearrange(ts, "(a b) -> a b", a=2)
...
def temperatures_average(temps: Tensor) -> Tensor:
    ...
    return einops.reduce(temps, "(a 7) -> a", reduction="mean")
...
def temperatures_differences(temps: Tensor) -> Tensor:
    ...
    """
    assert len(temps) % 7 == 0
    temps_rearrange = einops.rearrange(temps, "(a b) -> a b", b=7)
    temps_avg = einops.reduce(temps, "(a 7) -> a 1", "mean")
    return einops.rearrange(temps_rearrange - temps_avg, "a b -> (a b)")
...
# Or: use einops.repeat(temps_avg, "a -> (a 7)") and subtract it from temps
# %%
def temperatures_normalized(temps: Tensor) -> Tensor:
    ...
    """
    temps_avg = einops.reduce(temps, "(a 7) -> a", "mean")
    temps_std = einops.reduce(temps, "(a 7) -> a", t.std)
    return (temps - einops.repeat(temps_avg, "a -> (a 7)")) / einops.repeat(temps_std, "a -> (a 7)")
...
def normalize_rows(matrix: Tensor) -> Tensor:
    ...
    """
    return matrix / t.norm(matrix, dim=1, keepdim=True)  # keepdim=True for broadcasting
...
def cos_sim_matrix(matrix: Tensor) -> Tensor:
    ...
    matrix_norm = t.norm(matrix, dim=1, keepdim=True)
    denom = matrix_norm @ matrix_norm.T
    return matrix_mul / denom
...
def sample_distribution(probs: Tensor, n: int) -> Tensor:
    ...
    """
    probs_cum = t.cumsum(probs, dim=0)
    return t.sum((t.rand(n,1) >= probs_cum), dim=1)
...
def total_price_indexing(prices: Tensor, items: Tensor) -> float:
    ...
    """
    return prices[items].sum().item()  # .item() to return float value
...
def gather_2d(matrix: Tensor, indexes: Tensor) -> Tensor:
    ...
    assert matrix.ndim == indexes.ndim  # we need this
    assert matrix.shape[0] >= indexes.shape[0]  # # of rows in indexes cannot exceed that in matrix
...
def total_price_gather(prices: Tensor, items: Tensor) -> float:
    ...
    return t.sum(t.gather(prices, 0, items)).item()
...
def batched_logsumexp(matrix: Tensor) -> Tensor:
    ...
    """
    matrix_max, _ = t.max(matrix, dim=1)
    return matrix_max + t.log(t.sum(t.exp(matrix-matrix_max.unsqueeze(-1)),dim=1))
...
# Or: we may use rearrange() instead of unsqueeze() or squeeze()
# %%
def batched_softmax(matrix: Tensor) -> Tensor:
    ...
    """
    matrix_max, _ = t.max(matrix, dim=1, keepdim=True)
    matrix_exp = t.exp(matrix - matrix_max)  # also for numerical stability
    matrix_exp_sum = einops.reduce(matrix_exp, "b n -> b 1", "sum")
    return matrix_exp / matrix_exp_sum
...
def batched_logsoftmax(matrix: Tensor) -> Tensor:
    ...
    matrix_exp = t.exp(matrix - matrix_max)  # also for numerical stability
    matrix_exp_sum = einops.reduce(matrix_exp, "b n -> b 1", "sum")
    return t.log(matrix_exp / matrix_exp_sum)
...
def batched_cross_entropy_loss(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    return -t.log(t.gather(batched_softmax(logits), 1, true_labels.unsqueeze(-1)).squeeze(-1))
...
def einsum_trace(mat: np.ndarray):
    ...
    identity_matrix = t.eye(mat.shape[0])
    return einops.einsum(mat, identity_matrix, "i j, i j ->")
