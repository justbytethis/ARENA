...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        """
        super().__init__()
        self.weight = nn.Parameter((2 * t.randn(out_features, in_features) - 1) / in_features)
    ...
    def extra_repr(self) -> str:
        return f"w={list(self.weight.shape)},b={self.bias is not None}"
...
def train(args: SimpleMLPTrainingArgs) -> tuple[list[float], list[float], SimpleMLP]:
    ...
    for e in range(args.epochs):
        ...
        for imgs, labels in pbar:
            ...
            loss_list.append(loss.item())
            pbar.set_postfix(epoch=f"{e+1}/{args.epochs}", loss=f"{loss:.3f}")
        ...
        accuracy_list.append(acc / len(mnist_testset))
...
class Conv2d(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter((2 * t.rand(out_channels, in_channels, kernel_size, kernel_size) - 1) / np.sqrt(in_channels * kernel_size * kernel_size))
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return (x - einops.rearrange(mean, "n -> n 1 1")) / t.sqrt(einops.rearrange(var, "n -> n 1 1") + self.eps) \
            * einops.rearrange(self.weight, "n -> n 1 1") + einops.rearrange(self.bias, "n -> n 1 1")
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        return F.relu(self.left_block(x) + self.right_block(x))
...
class BlockGroup(nn.Module):
    def __init__(self, n_blocks: int, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.long = Sequential(
            *[ResidualBlock(out_feats,out_feats) for i in range(n_blocks-1)]
        )
...
class ResNet34(nn.Module):
    def __init__(
        ...
        self.bg = Sequential(
            *[BlockGroup(n_blocks_per_group[i], 64 if i == 0 else out_features_per_group[i-1], out_features_per_group[i], first_strides_per_group[i]) for i in range(len(n_blocks_per_group))]
        )
...
def predict(
    ...
    """
    model.eval()
    probs = t.softmax(model(images), -1)
    return probs.max(-1)
