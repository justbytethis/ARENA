...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    while len(broadcasted.shape) != len(original.shape):
        broadcasted = broadcasted.sum(0, keepdims=False)
    # Step 2: sum over dimensions where origianal has size 1
    for dim, shape in enumerate(original.shape):
        if shape == 1:
...
def forward_and_back(a: Arr, b: Arr, c: Arr) -> tuple[Arr, Arr, Arr]:
    ...
    df = log_back(final_grad_out, g, f)
    dd, de = multiply_back0(df, f, d, e), multiply_back1(df, f, d, e)
    
    # da, db, dc
    dg_dc = log_back(de, e, c)
    dg_da, dg_db = multiply_back0(dd, d, a, b), multiply_back1(dd, d, a, b)
...
class BackwardFuncLookup:
    def __init__(self) -> None:
        self.lookup_table: dict[tuple[Callable, int], Callable] = dict()
...
def log_forward(x: Tensor) -> Tensor:
    ...
    out = Tensor(
        np.log(x.array),
        x.requires_grad and grad_tracking_enabled
    )
    out.recipe = Recipe(
        np.log,
        (x.array,),
        {},
        {0: x}
    ) if out.requires_grad else None
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        ...
        requires_grad = is_differentiable and grad_tracking_enabled and \
            any([isinstance(a, Tensor) and a.requires_grad for a in args])
        out = Tensor(
...
# %%
# Skip this one, just paste the solution
class Node:
...
def backprop(end_node: Tensor, end_grad: Tensor | None = None) -> None:
    ...
    # Step 2: compute gradient in topological order
    for node in order:
        if node.is_leaf:
            ...
            if node.recipe is not None:  # Note: node.is_leaf == False -> node.require_grad = True
...
def maximum_back0(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    ...
    co = unbroadcast((x == out) - (x == y) / 2, x)
    return grad_out * co


def maximum_back1(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    """Backwards function for max(x, y) wrt y."""
    co = unbroadcast((y == out) - (x == y) / 2, y)
    return grad_out * co
...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        self.weight = Parameter((Tensor(np.random.rand(out_features, in_features) * 2 - 1) / np.sqrt(self.in_features)))
        if bias:
            self.bias = Parameter(Tensor(np.random.rand(out_features) * 2 - 1) / np.sqrt(self.in_features))
        else:
            self.bias = None
