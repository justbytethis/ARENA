...
def opt_fn_with_sgd(
    ...
    optim = t.optim.SGD((xy,), lr=lr, momentum=momentum)  # params should be iterable of Tensors
    trajectory = xy.detach().clone().unsqueeze(0)
    for _ in range(n_iters):
        loss = fn(xy[0], xy[1])
        loss.backward()
        optim.step()
        optim.zero_grad()
        this = xy.detach().clone().unsqueeze(0)
        trajectory = t.cat((trajectory, this), dim=0)
    return trajectory
...
class SGD:
    ...
    def step(self) -> None:
        for i, param in enumerate(self.params):
            update = param.grad
            if self.lmda != 0:
                update += self.lmda * param
            if self.mu != 0:
                self.b[i] = self.mu * self.b[i] + update
                update = self.b[i]
...
class Adam:
    ...
    def step(self) -> None:
        for i, param in enumerate(self.params):
            ...
            mbar = self.m[i] / (1 - self.beta1 ** self.t)
            vbar = self.v[i] / (1 - self.beta2 ** self.t)

            param -= self.lr * mbar / (t.sqrt(vbar) + self.eps)
        self.t += 1
...
class WandbResNetFinetuningArgs(ResNetFinetuningArgs):
    ...
    wandb_name: str | None = "0-3-resnet"
...
def update_args(args: WandbResNetFinetuningArgs, sampled_parameters: dict) -> WandbResNetFinetuningArgs:
    ...
    args.weight_decay = sampled_parameters["weight_decay_value"] if sampled_parameters["weight_decay_bool"] else 0
    return args
...
sweep_id = wandb.sweep(sweep=sweep_config, project="arena")
